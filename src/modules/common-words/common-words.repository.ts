...
class CommonWordsRepository {
	...
	public async getAllWords(): Promise<CommonWord[]> {
	...
	async findWordById(id: string): Promise<CommonWord | null> {
		const foundWord = await this.wordModel.findById(id).exec();
		return foundWord;
	}

	async updateWord(id: string, dto: UpdateCommonWordDto): Promise<CommonWord> {
